...
class WorkPackages::UpdateAncestors::Loader
  ...
  def descendants_of(work_package)
    @descendants ||= Hash.new do |hash, wp|
      hash[wp] = related_of(wp, :descendants).map do |d|
        ...
        # work_package
        #
        # where grandparent and parent is in ancestors.
        ...
        ancestors.detect { |ancestor| ancestor.id == d.id } || d
      end
    end
  ...
  def leaves_of(work_package)
    @leaves ||= Hash.new do |hash, wp|
      hash[wp] = related_of(wp, :leaves).each do |leaf|
  ...
  def related_of(work_package, relation_type)
    scope = work_package
              .send(relation_type)
